...
> 即使是 `const arr = []` 的写法，本质上是通过 `new Array()` 来创建的；<br/> `function fn() {}` 的写法，本质上是通过 `new Function()` 来创建的；<br/>
> 所有的函数，本质上也是对象
...
- <span style="color: red;font-weight: bold;">所有的对象</span>都有一个属性：`__proto__`，称之为 `隐式原型 `；默认情况下，它指向创建该对象的函数的原型，从而就有：
